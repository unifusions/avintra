                ...
                <x-card>
                    <form method="POST" action="{{ route('section.store') }}">
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-8">
                                <x-text-input :errorMsg="$errors->has('name')" id="name" class="" :value="old('name')"
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-8">
                                <x-text-input :errorMsg="$errors->has('description')" id="description" class="" :value="old('description')"
                                ...
                                <select class="form-select @if ($errors->has('division_id')) is-invalid @endif"
                                    ...
                                    @foreach ($divisions as $division)
                                        <option value="{{ $division->id }}"
                                            @if (old('division_id') == $division->id) selected @endif>{{ $division->name }}</option>
                                    @endforeach
                    ...
                    <tbody>
                        @foreach ($sections as $key => $section)
                            <tr>
                                ...
                                @canany(['update', 'view', 'delete'], $section)
                                    <td>
                                        <div class="action-button-container d-flex justify-content-evenly">
                                            {{-- <x-edit-button href=" {{ route('section.edit', $section) }} " /> --}}

                                            @can('delete', $section)
                                                <x-delete-button :model="$section" :modelId="$section->id" :modelName="$section->name"
                                                    action="{{ route('section.destroy', $section) }} " type="section" />
                                            @endcan
                                        </div>
                                    </td>
                                @endcanany
